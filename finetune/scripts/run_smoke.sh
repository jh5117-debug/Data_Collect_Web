...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash finetune/scripts/run_smoke.sh /absolute/path/to/vigil_dataset_export.zip" >&2
  exit 2
fi

DATA_ZIP="$1"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
CONFIG="${ROOT}/finetune/configs/smoke.yaml"
...
RUN_DIR="${ROOT}/finetune/runs/$(date -u +%Y%m%d_%H%M%S)_${FINGERPRINT}_smoke"
mkdir -p "${RUN_DIR}"
...
python "${ROOT}/finetune/scripts/run_qwen_text_baseline.py" --dataset-dir "${DATASET_DIR}" --run-dir "${RUN_DIR}" --allow-skip
...
python "${ROOT}/finetune/scripts/extract_qwen_encoder_features.py" --dataset-dir "${DATASET_DIR}" --config "${CONFIG}" --run-dir "${RUN_DIR}" --allow-skip
python "${ROOT}/finetune/scripts/train_stage2.py" --dataset-dir "${DATASET_DIR}" --config "${CONFIG}" --run-dir "${RUN_DIR}" --variant bce --allow-skip
python "${ROOT}/finetune/scripts/train_stage2.py" --dataset-dir "${DATASET_DIR}" --config "${CONFIG}" --run-dir "${RUN_DIR}" --variant bce_supcon --allow-skip

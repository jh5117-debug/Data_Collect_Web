...
class AssistantModelRuntime:
    ...
    def analyze_audio(self, audio_path: Path, calibration: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.inference is not None:
            result = self.inference.analyze(audio_path, run_transcript_after_trigger=True)
            ...
            return {
                "mode": self.mode,
                "rolling_transcript": result.get("transcript") or "",
                ...
                "debug": {"variant": result.get("variant"), "calibration_bias": bias},
            }

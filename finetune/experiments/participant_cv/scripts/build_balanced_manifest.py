...
def main() -> int:
    ...
    manifest_path = shared / "balanced_max100_manifest.jsonl"
    ...
    write_json(shared / "balanced_max100_summary.json", summary)
    prompt = Counter(row["prompt_group"] for row in rows)
    labels = Counter(str(row["label"]) for row in {r["clip_id"]: r for r in rows}.values())
    report = [
        ...
        "",
        "## Participant Cap Summary",
        "",
        "| Alias | Before | After | Removed |",
        "|---|---:|---:|---:|",
    ]
    for row in summary["participant_summary"]:
        report.append(f"| {row['participant_alias']} | {row['clips_before']} | {row['clips_after']} | {row['clips_removed']} |")
    text = "\n".join(report) + "\n"
    assert_public_text_is_sanitized(text)
    (reports / "BALANCED_MAX100_REPORT.md").write_text(text, encoding="utf-8")

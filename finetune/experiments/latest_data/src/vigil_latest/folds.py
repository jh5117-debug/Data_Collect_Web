...
def build_folds(clips: list[dict[str, Any]], *, fold_count: int = 5, seed: int = 20260620, starts: int = 200, weights: dict[str, float] | None = None) -> dict[str, Any]:
    weights = weights or {
        "participants": 5.0,
        "clips": 3.0,
        "positive": 2.0,
    ...
    for start in range(starts):
        ...
        for alias in order:
            ...
            for i in range(fold_count):
                ...
                candidates.append((fold_objective(tmp, weights), len(folds[i]), i))
    ...
    return {
        ...
        "folds": [{"fold": i, "participant_aliases": fold, "stats": best[2][i]} for i, fold in enumerate(best[1])],
    }

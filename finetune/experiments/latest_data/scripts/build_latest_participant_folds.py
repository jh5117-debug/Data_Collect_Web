...
def clip_rows(rows: list[dict]) -> list[dict]:
    seen = {}
    for row in rows:
        seen.setdefault(str(row["clip_id"]), dict(row))
...
def main() -> int:
    ...
    parser.add_argument("--config", default="finetune/experiments/latest_data/configs/latest_protocol.yaml")
    parser.add_argument("--out-dir", default="finetune/experiments/latest_data")
    args = parser.parse_args()
    config = yaml.safe_load(Path(args.config).read_text(encoding="utf-8"))
    rows = read_jsonl(args.manifest)
    clips = clip_rows(rows)
    folds = build_folds(clips, fold_count=int(config["fold_count"]), seed=int(config["seed"]))
    ...
    fold_path = shared / "latest_participant_folds_5fold.json"
    ...
    write_json(shared / "latest_shared_experiment_protocol.json", protocol)
    report = [
        "# Latest Fold Balance Report",
        "",
    ...
    (reports / "LATEST_FOLD_BALANCE_REPORT.md").write_text(text, encoding="utf-8")

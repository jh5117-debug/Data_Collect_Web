...
RUN_NAME="${3:-qwen3_asr_1_7b_full}"
TIMESTAMP="$(date -u +%Y%m%d_%H%M%S)"
LOG_DIR="$BENCH_ROOT/logs"
SAFE_RUN_NAME="$(echo "$RUN_NAME" | tr '/: ' '___' | tr -cd 'A-Za-z0-9_.-')"
LOG_PATH="$LOG_DIR/librispeech_full_${TIMESTAMP}_gpu${GPU_INDEX:-unset}.log"
RUN_DIR="$BENCH_ROOT/runs/${TIMESTAMP}_${SAFE_RUN_NAME}_full"
mkdir -p "$LOG_DIR"

if [[ -z "$GPU_INDEX" ]]; then
  echo "Usage: bash finetune/benchmarks/asr/scripts/run_librispeech_full.sh <physical_gpu_index> [model_name] [run_name]" >&2
  exit 2
fi
...
PY
  ...
  "$PY" "$BENCH_ROOT/scripts/run_qwen_librispeech.py" \
    --manifest "$BENCH_ROOT/manifests/all_test.jsonl" \
    --output-dir "$RUN_DIR" \
    --run-name "$RUN_NAME" \
    --model "$MODEL_NAME" \
    --resume \
